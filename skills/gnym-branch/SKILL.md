...
3. For a new Task, resolve the parent Story branch and confirm required prerequisite changes have been integrated there. A Task whose prerequisites are absent waits for integration; do not branch from a sibling Task or cherry-pick its changes to bypass the hierarchy. Verify local/remote Story divergence before selecting the base.
